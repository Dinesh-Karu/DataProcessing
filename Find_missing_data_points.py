...
log_file = "C:/_VAMK/_Coding/Data/Output/Log Missing data points of Hourly Avg Temp Vaasa 2020-2024.csv"
...
for _, row in df.iterrows():
    ...
    if (ref_year == input_data_dict[i]["Year"] and
        ...
        output_data.append([ref_year, ref_month, ref_day, ref_hour, 
                            "missing", 
                            "missing"])
        log_data.append([ref_year, ref_month, ref_day, ref_hour])
...
output_df = pd.DataFrame(log_data, columns=["Year", "Month", "Day", "Hour"])
output_df.to_csv(log_file, sep=",", index=False)
